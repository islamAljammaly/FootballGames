...
class Quick extends StatefulWidget {
  ...
  bool operator ==(Object other) {
    ...
    return other is Quick; // Check if the other object is of the same type
  }
...
class _QuickState extends State<Quick> {
  ...
  void generateRandomNumber() {
    ...
    randomNumber = random.nextInt(quizController.questions3.length);
  }

  void showNextQuestion() {
    setState(() {
      quizController.questions3.removeAt(randomNumber);
      quizController.currentQuestionIndex++;
      if (quizController.currentQuestionIndex == int.parse(quizController.dropdownValueDialog[quizController.challenges.indexOf('السرعة')])) {
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            quizController.currentQuestionIndex <
                    int.parse(quizController.dropdownValueDialog[quizController.challenges.indexOf('السرعة')])
                ? Column(
                    children: [
                      ...
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Text(
                              '${quizController.questions3[randomNumber].questionText}',
                              style:
                  ...
                  child: Center(
                    child: Text(
                      '${quizController.questions3[randomNumber].answers[0]}',
                      style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.bold,
                          fontSize: 20),
                ...
                : Column(
                  children: [
                    ...
                    quizController.challenges.length > quizController.challenges.indexOf('السرعة') + 1 ?
                    ElevatedButton(onPressed: (){quizController.scoreA = 0;
                      quizController.scoreB = 0;
                      quizController.currentQuestionIndex = 0;
                      Get.to(quizController.pages[quizController.challenges.indexOf('السرعة') + 1]);}, child: Text('انتقل إلى التحدي الاخر'))
                    : ElevatedButton(
    onPressed: () {
